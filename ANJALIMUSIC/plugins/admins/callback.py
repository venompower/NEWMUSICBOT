...
from ANJALIMUSIC.utils.database import get_assistant
from ANJALIMUSIC.utils.decorators.language import languageCB
from ANJALIMUSIC.utils.formatters import seconds_to_min
from ANJALIMUSIC.utils.inline import close_markup, stream_markup, stream_markup_timer
from ANJALIMUSIC.utils.stream.autoclear import auto_clean
from ANJALIMUSIC.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await ANJALI.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await ANJALI.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
